...
SESSION_SEARCH_GUIDANCE = (
    ...
    "decisions, and task outcomes; session_search/raw sessions for recent Hermes "
    "conversations; mempalace/raw archives for old ChatGPT/OpenClaw/Kai voice "
    "facts or exact private non-secret receipts; skills for procedures/workflows; "
    "web/current sources for current external facts; terminal/read_file/search_files "
    "for live system, file, path, git, process, or math facts; durable user/profile "
    "memory only for stable preferences and behavior invariants; "
    "Enzyme/Honcho/Holographic only when active/verified or explicitly queried. "
    "Secret/token requests are not normal retrieval: do not read raw credential "
    "files. In ordinary or intimate/presence turns, retrieve if needed but answer "
    "cleanly: do not mention memory/retrieval machinery, cite retrieval headers, or "
    "narrate negative space unless the user asks for memory/debug."
)
