...
from agent.mnemos_admission import ALLOWED_SYNTHETIC_SOURCE
from agent.mnemos_prompt_canary import (
    parse_mnemos_prompt_admission_config,
    retrieve_mnemos_prompt_packet,
)
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    _mnemos_prompt_block = _build_mnemos_prompt_admission_block(agent)
    if _mnemos_prompt_block:
        volatile_parts.append(_mnemos_prompt_block)
...
def _build_mnemos_prompt_admission_block(agent: Any) -> str:
    """Return an injected-test-only Mnemos canary block for the volatile tier.

    This hook is deliberately default-disabled: it only reads dependency-injected
    attributes placed on the agent by tests/canaries. It never reads real config,
    discovers MCP tools, opens Mnemos/Kai DBs, writes memory, or activates a live
    ...
    try:
        config = parse_mnemos_prompt_admission_config(raw_config)
        if not config.activation_possible:
            return ""

        packet = retrieve_mnemos_prompt_packet(
            config,
            query=getattr(agent, "_mnemos_prompt_admission_query", "Hermes continuity canary"),
            retrieve=getattr(agent, "_mnemos_prompt_admission_retriever", None),
        )
        _record_mnemos_prompt_packet_metadata(agent, packet)
    except Exception:
        return ""

    if packet.get("decision") not in {"admit", "summarize"}:
        return ""
    if packet.get("low_trust") is not True:
        return ""
    if packet.get("source") != ALLOWED_SYNTHETIC_SOURCE:
        return ""
...
def _record_mnemos_prompt_packet_metadata(agent: Any, packet: dict[str, Any]) -> None:
    """Store prompt-admission telemetry without retaining retrieved text.

    Gate-refresh evidence must not become a side channel for raw Mnemos rows or
    prompt-visible packets. Keep only bounded decision/telemetry fields.
    """

    try:
        metadata = {
            "decision": packet.get("decision"),
            "low_trust": packet.get("low_trust"),
            "source": packet.get("source"),
            "reason_codes": list(packet.get("reason_codes", [])),
            "telemetry": packet.get("telemetry", {}),
        }
        setattr(agent, "_mnemos_prompt_admission_last_packet_metadata", metadata)
    except Exception:
        pass
