...
def _build_mnemos_prompt_admission_retriever(*, expected_tool: str, max_limit: int):
    """Return a fail-closed retriever for the synthetic Mnemos prompt canary.

    The prompt hook remains dependency-injected: this closure is installed only
    after explicit profile config activation. It dispatches through the already
    registered MCP tool handler and normalizes the read-only wrapper's response
    into the adapter's expected ``tool/source/rows`` shape.
    """

    prefixed_tool = f"mcp_mnemos_ro_default_canary_{expected_tool}"

    def _retriever(request: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(request, dict):
            return _mnemos_retriever_reject("malformed_request", expected_tool)
        if request.get("tool") != expected_tool:
            return _mnemos_retriever_reject("tool_mismatch", expected_tool)
        if request.get("source") != "synthetic_shadow_sqlite":
            return _mnemos_retriever_reject("source_mismatch", expected_tool)
        if request.get("allow_live_db") is not False or request.get("allow_writes") is not False:
            return _mnemos_retriever_reject("unsafe_request", expected_tool)

        try:
            limit = int(request.get("max_items", max_limit))
        except Exception:
            limit = max_limit
        ...
        try:
            ...
            entry = registry.get_entry(prefixed_tool)
            if entry is None:
                return _mnemos_retriever_reject("mcp_tool_not_registered", expected_tool)
            raw_result = entry.handler({"query": str(request.get("query", "")), "limit": limit})
            payload = _extract_mnemos_mcp_payload(raw_result)
        except Exception:
            return _mnemos_retriever_reject("retrieval_error", expected_tool)

        if not isinstance(payload, dict):
            return _mnemos_retriever_reject("malformed_response", expected_tool)
        rows = payload.get("rows", payload.get("results", []))
        if not isinstance(rows, list):
            rows = []
        return {
            "tool": payload.get("tool", expected_tool),
            "source": payload.get("source"),
            "low_trust": payload.get("low_trust"),
            "rows": rows,
        }
...
def _extract_mnemos_mcp_payload(raw_result: Any) -> dict[str, Any] | None:
    """Unwrap registry/MCP handler JSON without trusting retrieved content."""
...
def _mnemos_retriever_reject(reason: str, expected_tool: str) -> dict[str, Any]:
    return {"tool": expected_tool, "source": "synthetic_shadow_sqlite", "rows": [], "error": reason}
...
def init_agent(
    ...
    # Default-disabled, dependency-injected Mnemos prompt-admission canary seam.
    # These stay None in normal runtime; tests/canaries may set both explicitly.
    agent._mnemos_prompt_admission_config = None
    agent._mnemos_prompt_admission_retriever = None
    agent._mnemos_prompt_admission_query = "Hermes continuity canary"
    agent._mnemos_prompt_admission_last_packet_metadata = None
    ...
    # Synthetic-only Mnemos prompt-admission canary activation.  The parser is
    # still fail-closed and this wiring only runs when the already-loaded
    # profile config explicitly enables the synthetic canary rails.  Retrieval
    # is dependency-injected into the prompt hook and routed through the
    # existing MCP tool registry entry; no live DB, writes, or tool discovery are
    # performed here.
    try:
        from agent.mnemos_prompt_canary import parse_mnemos_prompt_admission_config

        _mnemos_prompt_cfg = parse_mnemos_prompt_admission_config(_agent_cfg)
        if _mnemos_prompt_cfg.activation_possible:
            _mnemos_prompt_section = _agent_cfg.get("mnemos_prompt_admission", {})
            agent._mnemos_prompt_admission_config = {
                "mnemos_prompt_admission": _mnemos_prompt_section
            }
            if isinstance(_mnemos_prompt_section, dict) and isinstance(_mnemos_prompt_section.get("query"), str):
                agent._mnemos_prompt_admission_query = _mnemos_prompt_section["query"]
            agent._mnemos_prompt_admission_retriever = _build_mnemos_prompt_admission_retriever(
                expected_tool=_mnemos_prompt_cfg.mcp_tool,
                max_limit=_mnemos_prompt_cfg.max_items,
            )
    except Exception as _mpa_err:
        _ra().logger.warning("Mnemos prompt-admission canary disabled: %s", _mpa_err)
        agent._mnemos_prompt_admission_config = None
        agent._mnemos_prompt_admission_retriever = None
